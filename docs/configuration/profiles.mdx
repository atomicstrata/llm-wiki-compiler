...
Every read surface uses it: `llmwiki status`, JSON and OKF export, context packs,
the wiki index, lint, and the local viewer's page list, page header and graph
labels. A type that declares no `titleField` falls back to the literal `title`
key, and a page whose title field is missing, blank, or not a string falls back
to its slug — so adding `titleField` to an existing profile changes what those
surfaces display for its pages.
