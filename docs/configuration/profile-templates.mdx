...
An update journals the previous profile and advisory provenance before changing
either file. If the process is interrupted, `llmwiki template status` reports
...
Publishing a tap is a separate operator workflow. See [Publish a signed template
tap](/guides/publish-template-tap) for the package envelope, index, signing,
hosting, rotation, and client-verification procedure.
