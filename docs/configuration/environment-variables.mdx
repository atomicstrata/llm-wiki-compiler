...
Repair requires an embedding update with changed or pending eligible pages;
an unchanged no-op compile does not repair the index. Rebuilding an unavailable
index embeds the live eligible corpus again and can incur provider costs.
