...
```bash
ls .llmwiki/embeddings.json
```

If it's missing, embeddings haven't been built yet (they are created during `compile` when the provider supports them). Possible causes:
...
- **Semantic chunk retrieval** (`.llmwiki/embeddings.json` v2) narrows hundreds of pages down to a small top-K via cosine similarity before any LLM selection, with BM25 reranking on top.
