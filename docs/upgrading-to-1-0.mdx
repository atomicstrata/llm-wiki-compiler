...
This section describes the logical v2-to-v3 migration introduced in 1.0.
Current releases can store that same logical index in either `embeddings.json`
or `embeddings.bin`. Binary is detected without a flag and an old JSON backup
is not used while binary exists. Before returning to an older release, read
the [binary downgrade limitations](/configuration/environment-variables#embedding-storage);
unsetting the binary option is not a conversion.
