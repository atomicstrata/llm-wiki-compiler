...
Typed pages also show their immediate related entities, grouped by relation type and incoming, outgoing, or symmetric direction. Missing or invalid endpoints remain visible as unresolved rather than becoming broken links. Neighborhoods and source evidence are sorted and limited to 100 entries, with the full count shown when truncated. These descriptions are captured when the viewer starts; restart it after changing pages or relations.

The evidence section distinguishes a connector's metadata fetch from an original publication. A connector URL is not proof that publication bytes were stored. Explicit citations and source declarations link to raw ingested source entries at `#/_source/<filename>`; citation links carry physical line ranges, including frontmatter. Empty relation and evidence sections say when nothing is attached. Ordinary default-profile pages retain their existing layout.
...
Artifact details show the pinned reference, declared filename, verification health, manifest and declared JSON metadata fields. Every metadata, preview and download request rechecks the confined file, manifest, pinned SHA-256 and schema. A failed check suppresses bytes; the viewer never serves a different read after verification. Text and JSON preview as text, not executable HTML. Binary formats are not supported. Pages with no references say **No artifacts attached**; verification is bounded to the first 100 references with an explicit total when truncated.
