/**
 * @file src/local-workflows/stage-output.ts
...
/** Execution state held only for the duration of the host mutation callback. */
interface SubmitExecution {
...
// re-exported below so existing `from "./stage-output.js"` importers are unchanged.
...
async function applyPageOutput(
  ...
  await context.host.pages.validate(context.transaction, root, output);
  ...
  return preflightApplyRecord(root, run, stage, ref, async () => {
    await context.host.pages.apply(context.transaction, root, result.planned);
    return { decision: result.decision, outputRef: ref };
  }, undefined, (targetRoot, record) => context.host.records.write(context.transaction, targetRoot, record));
}
...
async function applyRelationOutput(
  ...
  guardTrustGatedNonPageWrite(runId, stage, projectId, "relation", context.host.authority.isTrustedWriteGranted);
  ...
  const recorded = await preflightApplyRecord(root, run, stage, worstRef, async () => {
    const applied = relationApplied(await context.host.domain.apply(context.transaction, root, [mutation]));
...
async function applyLifecycleOutput(
  ...
  guardTrustGatedNonPageWrite(runId, stage, projectId, "lifecycle-transition", context.host.authority.isTrustedWriteGranted);
  ...
  const lifecycleIntent = await prepareLifecycleIntent(root, output, context);
  let decision: TrustDecision = "allow";
  ...
  }, lifecycleIntent, runWriter(context));
  return { run: recorded, applied: true, decision };
}
...
async function recordStaged(
  ...
  await context.host.records.write(context.transaction, root, recorded);
  return recorded;
}
...
async function submitPageOutput(
  ...
  requireInScope(runId, stage, output.entityType);
  const result = await context.host.pages.plan(context.transaction, root, output);
  if (isApplyDecision(result.decision) && trustGateRequiresStaging(stage, projectId, context.host.authority.isTrustedWriteGranted)) {
    return { run: await recordStaged(context, root, run, stage, "stage-for-review"), applied: false, decision: "stage-for-review" };
  }
  if (isApplyDecision(result.decision)) {
    return { run: await applyPageOutput(context, root, run, stage, output, result), applied: true, decision: result.decision };
  }
  if (result.decision === "deny") throw new StageWriteDeniedError(runId, result.decision);
  return { run: await recordStaged(context, root, run, stage, result.decision), applied: false, decision: result.decision };
}
...
function dispatchOutput(
  ...
  if (output.kind === "human-input") {
    ...
    return recordHumanInputLocked(root, run, stage.id, stage.humanInput, output.input, context);
  }
  if (output.kind === "product-operation") {
    return recordProductOperationOutputLocked(root, run, stage, output, context);
  }
  if (output.kind === "page") return submitPageOutput(context, root, runId, run, stage, output, projectId);
  if (output.kind === "relation") return applyRelationOutput(context, root, runId, run, stage, output, projectId);
  if (output.kind === "lifecycle-transition") return applyLifecycleOutput(context, root, runId, run, stage, output, projectId);
  return applyArtifactOutput(root, runId, run, stage, output, projectId, origin, context);
}
...
async function dispatchOrAutoFail(
  ...
  try {
    return await dispatchOutput(context, root, runId, run, stage, output, projectId, origin);
  } catch (err) {
    if (!isHardDenial(err)) throw err;
    const failed = await markRunFailedLocked(root, run, err instanceof Error ? err.message : String(err),
      (projectRoot, terminal) => context.host.records.writeCandidates(
        context.transaction, projectRoot, terminalRunCandidates(terminal)));
 ...
 */
export async function submitStageOutputWithHost(
  host: LocalWorkflowHost,
  root: string,
  runId: string,
  output: StageOutput,
  ...
  try {
    result = await host.withMutation(root, transaction =>
      submitUnderLock({ host, transaction }, root, runId, captured, opts.origin ?? "workflow"), opts.lockOptions ?? {});
  } catch (err) {
  ...
  if (autoFailed !== undefined) {
    await maybeAutoProject(root, autoFailed.failedRun,
      (projectRoot, run) => host.projections.write(projectRoot, run.workflowId, projectRun(run)));
 ...
 */
async function submitUnderLock(context: SubmitExecution, root: string, runId: string, output: StageOutput, origin: WorkflowArtifactOrigin): Promise<SubmitResult> {
  const read = await context.host.history.read(root, runId);
  ...
  await context.host.profiles.assertProcessAuthority(root, read.run);
  if (isTerminalStatus(read.run.status)) throw new RunNotActiveError(runId, read.run.status);
  const { stage, projectId } = await resolveCurrentStage(root, read.run, context.host.profiles.load);
  assertStageAcceptsOutput(runId, stage, output);
  if (output.kind === "lifecycle-transition") {
    return submitLifecycleUnderLock(context, root, read.run, stage, output, projectId, origin);
  }
  if (output.kind === "product-operation" && Object.hasOwn(read.run.outputs, stage.id)) {
    return replayProductOperationOutputLocked(root, read.run, stage, output, context.host.observations);
  }
  if (output.kind !== "human-input") assertOutputNotApplied(runId, read.run, stage);
  return dispatchOrAutoFail(context, root, runId, read.run, stage, output, projectId, origin);
}

/** Recover the same lifecycle intent before revalidating a still-unapplied subject. */
async function submitLifecycleUnderLock(context: SubmitExecution, root: string, run: WorkflowRun, stage: WorkflowStageDef,
  output: LifecycleStageOutput, projectId: string, origin: WorkflowArtifactOrigin): Promise<SubmitResult> {
  const settled = await recoverLifecycleOutput(root, run, stage, output, context);
  if (settled !== null) return settled;
  await assertApprovedSubjectCurrent(root, run, stage, context.host);
  const { pendingOutput: _pending, ...retry } = run;
  assertOutputNotApplied(run.runId, retry, stage);
  return dispatchOrAutoFail(context, root, run.runId, retry, stage, output, projectId, origin);
}
