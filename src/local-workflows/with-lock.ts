/**
 * @file src/local-workflows/with-lock.ts
 ...
 * run the operation body, and ALWAYS release the lock. {@link withHostRunLock}
 ...
 * {@link withHostRunLock}, the `submitStageOutput` write path, AND the action surface's
 ...
 */
export function withHostRunLock<T>(host: LocalWorkflowHost, root: string, runId: string,
  body: (run: WorkflowRun, context: WorkflowExecutionContext) => Promise<T>,
  lockOptions: BlockingLockOptions = {}): Promise<T> {
  return host.withMutation(root, async transaction => {
    ...
    await host.profiles.assertProcessAuthority(root, read.run);
    return body(read.run, { host, transaction });
  }, lockOptions);
}
