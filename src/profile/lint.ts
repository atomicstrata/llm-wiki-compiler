 ...
 *
 *   5. Every hash-pinned artifactRef (page field or live relation attribute) is
 *      surfaced via {@link checkArtifactRefs}: a non-`ok` health (dangling /
 *      bytes-tampered / hash-mismatch / schema-invalid / store-unavailable) is
 *      an error; `artifact-unreadable` (cannot verify) is a warning.
 *
