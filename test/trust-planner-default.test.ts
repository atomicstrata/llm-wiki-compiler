 ...
 * (`planPageMutation` with a `{ kind: "raw" }` target in `src/trust/planner.ts`)
 * plus its executor round-trip. Default wiki pages carry Unicode `slugify` slugs (e.g.
...
function defaultArgs(
  slug: string,
  overrides: { directory?: string; allowOverwrite?: boolean } = {},
) {
  const { directory = "concepts", ...rest } = overrides;
  return {
    root,
    target: { kind: "raw" as const, directory, slug },
    body: GOOD_BODY,
    origin: "agent",
    reviewRouted: false,
    ...rest,
...
describe("planPageMutation (raw target) — Unicode slug, free target", () => {
  it("allows and plans one create with a RawPageRef (no id)", async () => {
    const out = await planPageMutation(defaultArgs(UNICODE_SLUG));
...
describe("planPageMutation (raw target) — existing target", () => {
  it("plans an update overwriting when allowOverwrite is true", async () => {
    ...
    const out = await planPageMutation(defaultArgs(UNICODE_SLUG, { allowOverwrite: true }));
    expect(out.decision).toBe("allow");
  ...
  it("blocks (no live mutation) when allowOverwrite is false", async () => {
    const target = path.join(root, "wiki", "concepts", `${UNICODE_SLUG}.md`);
    await writeFile(target, "OLD");
    const out = await planPageMutation(defaultArgs(UNICODE_SLUG, { allowOverwrite: false }));
...
describe("planPageMutation (raw target) — unsafe filename components are blocked", () => {
  ...
  for (const slug of unsafe) {
    it(`blocks slug ${JSON.stringify(slug)} with no live mutation`, async () => {
      const out = await planPageMutation(defaultArgs(slug));
      expect(out.planned).toEqual([]);
  ...
  it("blocks an unsafe directory component", async () => {
    const out = await planPageMutation(defaultArgs(UNICODE_SLUG, { directory: "../evil" }));
    expect(out.planned).toEqual([]);
  });
});
