...
function document(answer: string): string {
  return buildQueryDocument("[[title-only]]", answer, "2026-09-19T00:00:00Z").document;
}
...
it.each(["direct", "proposal", "approval"] as const)("%s accepts resolved and empty reports", async (mode) => {
  expect((await validateCitationPublication(ctx.dir, document("[[Alpha]]"), mode)).citations)
    .toEqual([{ target: "alpha", status: "resolved", pageId: "concepts/alpha" }]);
  expect(await validateCitationPublication(ctx.dir, document("`[[code-only]]`"), mode))
    .toEqual({ version: 1, citations: [] });
});

it.each(["direct", "approval"] as const)("%s refuses pending targets", async (mode) => {
  await expect(validateCitationPublication(ctx.dir, document("[[BETA]] [[beta]]"), mode))
...
it("proposal accepts pending observations without granting publication", async () => {
  expect((await validateCitationPublication(ctx.dir, document("[[beta]]"), "proposal")).citations)
...
it.each(["direct", "proposal", "approval"] as const)("%s refuses broken targets with exact diagnostics", async (mode) => {
  await expect(validateCitationPublication(ctx.dir, document("[[BETA]] [[Gone]] [[GONE]]"), mode))
...
it("keeps exact-slug precedence over aliases and retained precedence over pending", async () => {
  ...
  expect((await validateCitationPublication(ctx.dir, document("[[Beta]] [[Query Exact]]"), "direct")).citations)
...
it("refuses a typed-only target rather than treating it as retained", async () => {
  ...
  await expect(validateCitationPublication(ctx.dir, document("[[Typed Only]]"), "direct"))
