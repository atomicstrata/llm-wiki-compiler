...
import { symlink, mkdir, readFile, readdir, rename } from "fs/promises";
...
describe("atomicWrite ancestor-symlink confinement", () => {
  ...
  it("fails closed when the parent is swapped after confinement", async () => {
    const { root, outside } = ctx;
    const privateDir = path.join(root, ".llmwiki");
    await mkdir(privateDir, { recursive: true });
    const target = path.join(privateDir, "profile.json");

    await expect(atomicWrite(target, "secret", {
      confineRoot: root,
      afterParentCheckForTest: async () => {
        await rename(privateDir, path.join(root, ".llmwiki.real"));
        await symlink(outside, privateDir, "dir");
      },
    })).rejects.toThrow();
