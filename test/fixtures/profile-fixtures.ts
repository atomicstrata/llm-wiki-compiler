...
/**
 * A gated research profile: an `experiments.stage` lifecycle whose `complete`
 * state requires `minCount` `tests`→`ideas` relations on the experiment's `from`
 * side. `complete` declares no evidence requirement, so the relation precondition
 * is the SOLE gate on entering it. Shared by the write-side enforcement suite and
 * the read-side standing-invariant suite so the one gated shape is spelled once.
 *
 * @param minCount - The required distinct-existing relation count (default 1).
 * @returns The gated {@link ProfilePack}.
 */
export function gatedResearchProfile(minCount = 1): ProfilePack {
  return {
    schemaVersion: 1,
    profileId: "gated-research",
    entities: {
      ...
      experiments: {
        ...
        lifecycle: {
          ...
          transitionRelationRequirements: {
            complete: [{ relationType: "tests", role: "from", otherTypes: ["ideas"], minCount }],
          },
        },
      },
    },
...
/**
 * The resolved `loadProfile` shape for the DEFAULT (empty-entities) profile.
 * Shared by the consumer/retrieval tests that `vi.mock("../src/profile/load.js")`
 * so they don't each re-spell the same `{ profile, loadedFrom, digest }` literal
 * (fallow flagged the repeated factory body as a clone).
 */
export function defaultProfileLoad(): { profile: ProfilePack; loadedFrom: null; digest: string } {
  return { profile: { schemaVersion: 1, profileId: "default", entities: {} }, loadedFrom: null, digest: "" };
}
...
/**
 * Create an isolated tmp project root, materialize a research-lite project in it,
 * and (optionally) overwrite its on-disk profile with `pack`. The shared
 * `beforeEach` shape for the relation/lifecycle SDK suites — both stand up a
 * research-lite root and write a tailored profile the same way, so this is the one
 * definition of that setup. Pair with a `rm(root, { recursive, force })` teardown.
 *
 * @param prefix - tmp-dir name prefix for the project root.
 * @param pack - Optional profile to write over the default research-lite profile.
 * @returns The absolute project root path.
 */
export async function makeResearchLiteProjectRoot(prefix: string, pack?: ProfilePack): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), prefix));
  await buildResearchLiteProject(root);
  if (pack) await writeFile(path.join(root, PROFILE_FILE), JSON.stringify(pack), "utf8");
  return root;
}
