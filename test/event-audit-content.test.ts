 ...
 * `relation-compact` audit event (A5), lifecycle-transition events now record the
 * composed trust decision TOP-LEVEL (the lifecycle kind is a real executor
 * authority that composes a TrustDecision over the final frontmatter), and the
 * chain still verifies across a mix of relation-create + relation-compact +
 * lifecycle events.
 */
...
describe("lifecycle transitions record the composed trust decision TOP-LEVEL", () => {
  it("emits a well-formed lifecycle-transition event carrying decision=allow", async () => {
