/** Portable OKF identifiers are resolved beneath a native absolute output root. */
...
const OUTPUT_ROOT = path.resolve("out");
...
describe("resolveOutputPaths", () => {
  it("native pages use slug-paths; a safe foreign okfPath is restored", () => {
    ...
    const { paths, warnings } = resolveOutputPaths(pages, OUTPUT_ROOT);
  ...
  const expectCollisionRejected = (a: ExportPage) => {
    ...
    const { paths } = resolveOutputPaths([a, b], OUTPUT_ROOT);
  ...
  it("isSafeOkfPath rejects reserved/escape/absolute, allows nested index.md", () => {
    expect(isSafeOkfPath("index.md", OUTPUT_ROOT)).toBe(false);
    expect(isSafeOkfPath("references/x.md", OUTPUT_ROOT)).toBe(false);
    expect(isSafeOkfPath("/abs.md", OUTPUT_ROOT)).toBe(false);
    expect(isSafeOkfPath("a/../../b.md", OUTPUT_ROOT)).toBe(false);
    expect(isSafeOkfPath("./index.md", OUTPUT_ROOT)).toBe(false);
    expect(isSafeOkfPath("a/./b.md", OUTPUT_ROOT)).toBe(false);
    expect(isSafeOkfPath("tables/index.md", OUTPUT_ROOT)).toBe(true);
    expect(isSafeOkfPath("tables/customers.md", OUTPUT_ROOT)).toBe(true);
  });
  it("isSafeOkfPath rejects URL-unsafe characters (spaces/parens would yield malformed links)", () => {
    expect(isSafeOkfPath("tables/my customers.md", OUTPUT_ROOT)).toBe(false); // space truncates link URL
    expect(isSafeOkfPath("report (2024).md", OUTPUT_ROOT)).toBe(false);       // paren closes link early
    expect(isSafeOkfPath("tables/customers.md", OUTPUT_ROOT)).toBe(true);     // still safe
    expect(isSafeOkfPath("a/../../b.md", OUTPUT_ROOT)).toBe(false);           // still rejected
  });
  it("isSafeOkfPath rejects non-.md / directory-like targets (importer only round-trips .md docs)", () => {
    expect(isSafeOkfPath("tables/customers", OUTPUT_ROOT)).toBe(false); // no extension
    expect(isSafeOkfPath("tables/", OUTPUT_ROOT)).toBe(false);          // directory-like
    expect(isSafeOkfPath("tables/customers.md", OUTPUT_ROOT)).toBe(true);
  });
  ...
  it("two foreign pages sharing an okfPath resolve deterministically regardless of input order", () => {
    ...
    const forward = resolveOutputPaths([a, b], OUTPUT_ROOT).paths;
    const reversed = resolveOutputPaths([b, a], OUTPUT_ROOT).paths;
