...
function visibleSlugs(dom: import("jsdom").JSDOM): string[] {
  const links = dom.window.document.querySelectorAll(
    ".list-row a.list-title",
  );
  return Array.from(links).map((a) => a.getAttribute("href")?.replace(/^#\//, "") ?? "");
}
...
describe("public concept-list freshness filter", () => {
  it("renders the filter <select> with every axis option", async () => {
    const { dom } = await mountViewerDom(pagesResponderFor(MIXED_ROWS), "#/concepts");
